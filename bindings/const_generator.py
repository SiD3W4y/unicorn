...
template = {
    ...
    'snapsnap' : {
            'header' : "// For snapsnap, autogenerated file.\nnamespace ssnap {\nnamespace arch {\nnamespace %s {\n",
            'footer' : '}\n}\n',
            'line_format': "constexpr int %s = %s;\n",
            'out_file': "./snapsnap/%s.hh",
            # prefixes for constant filenames of all archs - case sensitive
            'arm.h': "arm",
            'arm64.h': "arm64",
            'mips.h': "mips",
        ...
        for line in lines:
            ...
            for t in tmp:
                ...
                if f[0].startswith("UC_" + prefix.upper()):
                    ...
                    # snapsnap specific hacks
                    if lang == "snapsnap" and target != "unicorn.h":
                        # We don't support instruction hooks in snapsnap
                        if "_INS_" in lhs_strip:
                            continue

                        chunks = lhs_strip.split('_')
                        lhs_strip = "_".join(chunks[2:]).lower()
